...
public class OrderServiceProcess implements OrderService {
	...
	public void addGoods(SaveCateDTO dto) {
		...
		CartItemEntity cartItem = cir.findByCartEntityNoAndGoodsNo(cart.getNo(), goods.getNo()); // 카트 넘버와 상품 넘버로 카트아이템
																									// 엔티티 찾아냄

		// 상품이 장바구니에 존재하지 않는다면 카트상품 생성 후 추가
		if (cartItem == null) {
			cartItem = CartItemEntity.createCartItem(cart, goods, 1);
			cir.save(cartItem);
		}
	...
	@Override
	public void itemDelete(SaveCateDTO dto) {
		MemberEntity me = mr.findByEmail(dto.getEmail()).get();// 세션의 이메일 정보로 유저 no값 가져오기
		Optional<CartEntity> cart2 = cr.findByMemberEntityNo(me.getNo());// 유저 no값으로 연결된 카트엔티티가 있는지 없는지 확인하기
		CartEntity cart;
		if (cart2.isEmpty()) {// 없으면 만들어서 저장
	...
	@Transactional
	@Override
	public void deleteAll(SaveCateDTO dto) {
		MemberEntity me = mr.findByEmail(dto.getEmail()).get();// 세션의 이메일 정보로 유저 no값 가져오기
		CartEntity cart= cr.findByMemberEntityNo(me.getNo()).orElseThrow();// 유저 no값으로 연결된 카트엔티티가 있는지 없는지 확인하기

		cir.deleteAllByCartEntity(cart);
